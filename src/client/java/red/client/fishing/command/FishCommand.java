...
public class FishCommand {
    ...
    public static void register(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        dispatcher.register(ClientCommandManager.literal("fish")
                ...
                .then(ClientCommandManager.literal("cinderbat")
                        .then(ClientCommandManager.argument("enabled", StringArgumentType.word())
                                .executes(FishCommand::setCinderbatHighlight)))
                .then(ClientCommandManager.literal("cinderdebug")
                        .then(ClientCommandManager.argument("enabled", StringArgumentType.word())
                                .executes(FishCommand::setCinderbatDebug)))
    ...
    private static int setCinderbatHighlight(CommandContext<FabricClientCommandSource> context) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.player == null) {
            return 0;
        }
        ...
        CinderbatHighlightConfig.setEnabled(enabled);
        sendMessage("Cinderbat Highlight: " + (enabled ? "ENABLED" : "DISABLED"), enabled ? Formatting.GREEN : Formatting.YELLOW);
        sendMessage("Status: " + CinderbatHighlight.getStatusText(), Formatting.AQUA);
        return 1;
    }

    private static int setCinderbatDebug(CommandContext<FabricClientCommandSource> context) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.player == null) {
            return 0;
        }
        ...
        CinderbatHighlightConfig.setDebugEnabled(enabled);
        sendMessage("Cinderbat Debug: " + (enabled ? "ENABLED" : "DISABLED"), enabled ? Formatting.GREEN : Formatting.YELLOW);
        return 1;
    }
    ...
    private static int showStatus(CommandContext<FabricClientCommandSource> context) {
        ...
        sendMessage("Cinderbat Highlight: " + (CinderbatHighlightConfig.isEnabled() ? "ENABLED" : "DISABLED"),
                CinderbatHighlightConfig.isEnabled() ? Formatting.GREEN : Formatting.RED);
        sendMessage("Cinderbat Status: " + CinderbatHighlight.getStatusText(), Formatting.YELLOW);
        sendMessage("Cinderbat Debug: " + (CinderbatHighlightConfig.isDebugEnabled() ? "ENABLED" : "DISABLED"),
                CinderbatHighlightConfig.isDebugEnabled() ? Formatting.GREEN : Formatting.GRAY);
