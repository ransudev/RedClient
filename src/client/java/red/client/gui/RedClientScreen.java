...
public class RedClientScreen extends BaseOwoScreen<FlowLayout> {
    ...
    private void buildTitleSection(FlowLayout rootComponent) {
        FlowLayout titleSection = (FlowLayout) UIContainers.verticalFlow(Sizing.fill(100), Sizing.content())
                .horizontalAlignment(HorizontalAlignment.CENTER)
                .margins(Insets.bottom(10));

        // Main title
        titleSection.child(
                UIComponents.label(Text.of("RedClient GUI"))
        ...
        // Subtitle
        titleSection.child(
                UIComponents.label(Text.of("Hunting & Fishing Features"))
                        .horizontalTextAlignment(HorizontalAlignment.CENTER)
        ...
        // Help text
        titleSection.child(
                UIComponents.label(Text.of("Configure your macros and features"))
                        .horizontalTextAlignment(HorizontalAlignment.CENTER)
    ...
    private void buildTabBar(FlowLayout rootComponent) {
        ...
        this.schedulerTabButton = (ButtonComponent) UIComponents.button(
                Text.literal(Module.SCHEDULER.label),
                button -> switchModule(Module.SCHEDULER)
        ).horizontalSizing(Sizing.fixed(130));
        
        this.huntingTabButton = (ButtonComponent) UIComponents.button(
                Text.literal(Module.HUNTING.label),
                button -> switchModule(Module.HUNTING)
        ).horizontalSizing(Sizing.fixed(130));
        
        this.fishingTabButton = (ButtonComponent) UIComponents.button(
    ...
    private void buildBottomButtons(FlowLayout rootComponent) {
        FlowLayout buttonRow = (FlowLayout) UIContainers.horizontalFlow(Sizing.fill(90), Sizing.content())
                .gap(8)
                .horizontalAlignment(HorizontalAlignment.CENTER)
        ...
        // Refresh button
        buttonRow.child(
                UIComponents.button(Text.literal("Refresh"), button -> {
                    refreshSections();
        ...
        // Done button
        buttonRow.child(
                UIComponents.button(Text.literal("Done"), button -> this.close())
                        .horizontalSizing(Sizing.fixed(100))
        );
